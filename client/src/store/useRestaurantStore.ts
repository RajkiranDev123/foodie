...
export const useRestaurantStore = create<RestaurantState>()(persist((set, get) => ({
    ...
    createRestaurant: async (formData: FormData) => {
        try {
            set({ loading: true });
            const response = await axiosInstance.post(`api/v1/restaurant`, formData, {
                headers: {
    ...
    getRestaurant: async () => {
        try {
            set({ loading: true });
            const response = await axiosInstance.get(`api/v1/restaurant`);
            if (response.data.success) {
    ...
    updateRestaurant: async (formData: FormData) => {
        try {
            set({ loading: true });
            const response = await axiosInstance.put(`api/v1/restaurant`, formData, {
                headers: {
                    'Content-Type': 'multipart/form-data'
                }
            });
    ...
    searchRestaurant: async (searchText: string, searchQuery: string, selectedCuisines: any) => {
        try {
            ...
            const response = await axiosInstance.get(`api/v1/restaurant/search/${searchText}?${params.toString()}`);
            if (response.data.success) {
    ...
    getSingleRestaurant: async (restaurantId: string) => {
        try {
            const response = await axiosInstance.get(`api/v1/restaurant/${restaurantId}`);
            if (response.data.success) {
    ...
    getRestaurantOrders: async () => {
        try {
            const response = await axiosInstance.get(`api/v1/restaurant/order`);
            if (response.data.success) {
    ...
    updateRestaurantOrder: async (orderId: string, status: string) => {
        try {
            const response = await axiosInstance.put(`api/v1/restaurant/order/${orderId}/status`, { status }, {
                headers: {
